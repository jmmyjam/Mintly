...
venv/bin/python scripts/snapshot_all.py          # snapshot every card's price into history (daily job; --max-pages N to limit)
```
...
- `Backend/app/services/price_history.py` — shared snapshot store: `extract_price`, `record_snapshots` (one row per card per UTC day), `previous_prices` (most recent prior-day snapshot per card), `annotate_price_changes` (adds `priceChange` to card dicts), `card_history`. Used by both the cards and portfolio routers — snapshots are recorded for ANY browsed card, not just held ones.
