...
A LaunchAgent (`~/Library/LaunchAgents/com.mintly.daily-snapshot.plist`) runs `snapshot_all.py` daily at 6am via `run_daily_snapshot.sh`, logging to `~/Library/Logs/mintly-daily-snapshot.log` — see HANDOFF for load/unload/kickstart commands. It needs the Postgres in `DATABASE_URL` running; a full run is ~20k cards / ~1h (upstream is slow).
...
- `Backend/snapshot_all.py` — standalone daily job: pages the full Pokemon TCG card list (`select=id,tcgplayer`) and records a `card_price_snapshot` for every priced card, so history is gap-free instead of only covering browsed/held cards. Idempotent (per-UTC-day dedupe via `record_snapshots`). Talks straight to the DB + upstream — the app need not be running. `run_daily_snapshot.sh` is the launchd wrapper.
