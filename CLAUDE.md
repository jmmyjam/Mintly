...
- Price-variant preference order (keep backend `extract_price` in `price_history.py` and frontend `getCardPrice` in sync): holofoil → normal → reverseHolofoil → 1stEditionHolofoil, using `mid`.
