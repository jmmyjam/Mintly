...
- `card_price_snapshot` — one row per card per UTC day: card_id, price, snapshot_date. Shared across users, with a composite `(card_id, snapshot_date)` index. Written by the daily `scripts/snapshot_all.py` job (every card), and also whenever any user loads their portfolio **or browses a priced card** through a card endpoint (all deduped per UTC day). It is the app-wide price-history store; portfolio value-over-time is derived from it by filtering to a user's holdings (`/portfolio/history`), so there is no separate portfolio-snapshot table. (Renamed from `portfolio_snapshot` — migration `b7e1c4d9f2a3` renames the table and its indexes in place, preserving rows.)
...
| `GET /search?q=&page=` | Smart search — see below. Returns `{data, page, pageSize, totalCount}` (50/page); priced cards carry a `priceChange`. |
...
| `GET /cards/{card_id}` | Single card; drives the detail page. Cached (6h fresh, then stale-while-revalidate). Carries `priceChange` when a prior snapshot exists. |
...
3. **Frontend tests** — the backend routers are covered (`Backend/tests/`, 86 tests); the React pages are not.
