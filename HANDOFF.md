...
- `card_price_snapshot` — one row per card per UTC day: card_id, price, snapshot_date. Shared across users, with a composite `(card_id, snapshot_date)` index. Written whenever any user loads their portfolio **or browses a priced card** through a card endpoint (deduped per UTC day). It is the app-wide price-history store; portfolio value-over-time is derived from it by filtering to a user's holdings (`/portfolio/history`), so there is no separate portfolio-snapshot table. (Renamed from `portfolio_snapshot` — migration `b7e1c4d9f2a3` renames the table and its indexes in place, preserving rows.)
...
- **Per-card price history**: `/cards/{id}/history` returns Mintly's own daily snapshots (there is no upstream history endpoint), so a card's chart is only as deep as how long Mintly has been recording it — sparse for cards seen for the first time. The default window is ~5 years; the chart shows a "not enough history yet" note with fewer than two points.
...
2. **Scheduled snapshots** — snapshots now record for any browsed card, not just portfolio visits, so history coverage is broader; but it's still visit-dependent, so a daily cron over tracked cards would make the history/portfolio charts gap-free.
